...
from app.database.models import  Uncountable, Preposition, Words, Users, Question
...
async def test(listt= None, firstans= '', secondans= ''):
    async with async_session() as session:
        if not listt:
            listt = ['a0', 'b1', 'c2', 'd3', 'a0', 'b1', 'c2', 'd3']
            listt2 = listt[4:]
            listt = listt[:4]
        ...
        for i in range(4):
            keyboard.add(InlineKeyboardButton(text = listt[i][:-1], callback_data= Testt(keyboard=x, firstans= listt[i], secondans=secondans).pack()))
            keyboard.add(InlineKeyboardButton(text = listt[4 + i][:-1], callback_data= Testt(keyboard=x, firstans=firstans, secondans= listt[4 + i]).pack()))
...
async def retrieve_rate(id):
    async with async_session() as session:
        ...
        for tupl in sentense:
            for t1 in tupl:
                listt.append(t1)
        return f"Stars: {listt[0]}\nLevel: {listt[1][0]} | {str(int(float(listt[1])*100))[-2:]}%"
