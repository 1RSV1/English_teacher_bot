...
async def retrieve_questionlevel_questiontime(id):
    async with async_session() as session:
        listt = []
        questionlevel = await session.execute(select(Users.questionlevel, Users.questiontime).where(Users.tg_id == id))
        for tupl in questionlevel:
            for t1 in tupl:
                listt.append(t1)
        return listt    

async def update_questionlevel_questiontime(questionlevel, questiontime, rightans,  tg_id):
    async with async_session() as session:
        listt = []
        info = await session.execute(select(Users.stars, Users.level).where(Users.tg_id == tg_id))
        for tupl in info:
            for t1 in tupl:
                listt.append(t1)
        await session.execute(update(Users).where(Users.tg_id == tg_id).values(stars = str(int(listt[0])+ rightans), level = str(float(listt[1]) + rightans/100), questionlevel=str(questionlevel), questiontime = questiontime))
