...
async def update_stars_level_wordslevel_wordstime(stars, level, wordslevel, wordstime, rightans,  tg_id):
    async with async_session() as session:
        #listt = []
        #info = await session.execute(select(Users.stars, Users.level).where(Users.tg_id == tg_id)) добавь старс и левел в параметры
        #for tupl in info:
            #for t1 in tupl:
                #listt.append(t1)
        await session.execute(update(Users).where(Users.tg_id == tg_id).values(stars = str(int(stars)+ rightans), level = str(float(level) + rightans/100), wordslevel=str(wordslevel), wordstime = wordstime))
        await session.commit()        
