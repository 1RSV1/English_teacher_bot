...
async def test(table, id, listt='', firstans= '', secondans= '', rightans= 0, falseans= 0):
    #async with async_session() as session:
...
async def retrieve_datalist(table, id):
    async with async_session() as session:
        ...
        for key, value in db_string.items():
            if value == None or type(value) == int or key.startswith('_'):
                continue
            listt.append(value)
        for i in range(len(listt)):
            listt[i] += str(i)   
...
# Question queries---------------------------------------------------------------------------------------------------------------
async def collect_words(id, rightans= 0, list = None, indexes = None, current_indexes = '', table = None):
    #async with async_session() as session:
        list = await retrieve_datalist(table, id)
        if indexes:
            sorted_list = []
...
async def retrieve_rate(id):
    async with async_session() as session:
        ...
        return f"Stars: {str(listt[0])}\nLevel: {level[0]} | {int(level[1])}%" # тут ошибка индекса
