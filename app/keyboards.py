...
async def two_options(leveloflesson):
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text="повторить пройденное", callback_data=rq.Variable(level= leveloflesson).pack()))
    keyboard.add(InlineKeyboardButton(text="открыть за 20⭐", callback_data='gg'))
    return keyboard.adjust(2).as_markup()

async def completed_lessons(leveloflesson):
    keyboard = InlineKeyboardBuilder()
    for level in range(int(leveloflesson)):
        keyboard.add(InlineKeyboardButton(text=str(level + 1), callback_data=rq.Variable2(level= str(level) + '1').pack()))
