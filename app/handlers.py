...
import datetime
from app.database.requests import MyCallback, Questions, MyCallback3
...
async def compilesentence(message: Message):
    questionlevel = await rq.retrieve_questionlevel_questiontime(message.from_user.id)
    if datetime.datetime.now().strftime('%x') > questionlevel[1]:
        questionlevel = questionlevel[0] + '1'
...
async def my_callback(query: CallbackQuery, callback_data: MyCallback):
    ...
    if word in query.message.text:
        ...
        await query.message.edit_text(text= callback_data.word + ' ?' if query.message.text == ' ?'  else query.message.text.rstrip('?') + callback_data.word + ' ?' , reply_markup= await rq.inlinebuttons2(x, callback_data.numberofquestion, callback_data.rightans))
        ...
        if len(listMessage) == len(x):
            ...
            if listMessage == rightans:
                ...
                if str(callback_data.numberofquestion)[-1] == '0':
                    print(callback_data.numberofquestion)
                    questionlevel = await rq.retrieve_questionlevel_questiontime(query.from_user.id) 
                    print(questionlevel)
                    if str(callback_data.numberofquestion)[:-1] == str(int(questionlevel[0]) + 1): # будет ли проблема при переходе с 10 на 11 уровень? 
                        callback_data.rightans += 1
                        await rq.update_questionlevel_questiontime(int(questionlevel[0]) + 1, datetime.datetime.now().strftime('%x'), callback_data.rightans, query.from_user.id)
                    else:
                         await query.message.answer('ты уже проходил этот урок')   
                ...
                if str(callback_data.numberofquestion)[-1] == '0':
                    questionlevel = await rq.retrieve_questionlevel_questiontime(query.from_user.id) 
                    if str(callback_data.numberofquestion)[:-1] == str(int(questionlevel[0]) + 1):
                        await rq.update_questionlevel_questiontime(int(questionlevel[0]) + 1, datetime.datetime.now().strftime('%x'), callback_data.rightans, query.from_user.id)
                    else:
                         await query.message.answer('ты уже проходил этот урок')   
                    await query.message.edit_text('урок закончен. плюс сколько то денег')
