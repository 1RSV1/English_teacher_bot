...
from app.database.models import  Preposition, Words, Users, Test2, Test
...
async def handle_all_audios(message: Message):
    cat = FSInputFile("file_example_MP3_700KB.mp3", filename = 'gg')
    await rq.db_helper(4444)
    print(cat)
    file_id = 'https://file-examples.com/storage/fe4a1fdc54682dc65a37ab9/2017/11/file_example_OOG_1MG.ogg'
    await bot.send_voice(chat_id = 155269575, voice = cat)
...
async def compilesentence(message: Message):
    task_level = True #await rq.db_helper(message.from_user.id, task_param = 'words_level_time', command= 'conditionals')
    if task_level:
