...
@router.message(Command('poll'), flags={'chat_action': 'typing', 'rate_limit': {'rate': 5}})
@flags.chat_action(initial_sleep=0, action="typing", interval=3)
async def cmd_poll(message: Message, counter: int):
...
@router.poll()
async def poll_answer_handler(poll: Poll, counter: int):
...
@router.poll_answer()
async def poll_answer_handler(poll_answer: PollAnswer, counter: int):
