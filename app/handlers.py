...
import datetime
from app.database.requests import Prepositions, Questions, MyCallback3, Variable, Variable2
...
async def my_callback(query: CallbackQuery, callback_data: Variable):
    await query.message.edit_text('выбери номер задания', reply_markup= await kb.completed_lessons(callback_data.level))
# Questions logic -------------------------------------------------------------------------------------------------------------------------
...
async def compilesentence(message: Message):
    ...
    if datetime.datetime.now().strftime('%x') > data[3]:
        ...
        await message.answer('Сегодня ты уже прошел тест. Новый появится завтра', reply_markup= await kb.two_options(data[2]))
