...
import datetime
from app.database.requests import Prepositions, Questions, MyCallback3, Variable, Variable2, Payment, Conditionals, Control
from app.database.models import async_session
from app.database.models import  Preposition, Words, Users, Test2, Test, Question
...
async def handle_all_audios(message: Message):
    cat = FSInputFile("ttsmaker-file-2025-5-26-15-1-21.opus", filename = 'gg')
    #url = 'https://httpbin.org/anything'
    #myobj = {'somekey': 'somevalue'}
    await bot.download(message.voice.file_id, f'audio/{message.from_user.id}.oga')
    cat = FSInputFile(f'audio/{message.from_user.id}.oga', filename = 'gg')
    #x = requests.post(url, json = myobj)
    #print(File)
    #await rq.db_helper(4444)
...
async def cmd_poll(message: Message):
    explanation = '1️⃣' # из бд
    #open_period = 600
    #await message.answer_poll(question= 'newer'[:3] + str(message.from_user.id) , options= [input_poll_option.InputPollOption(text= 'impossible', voter_count = 10), input_poll_option.InputPollOption(text= 'inappropriate', voter_count= 10), input_poll_option.InputPollOption(text= 'inevitable', voter_count= 10)], correct_option_id= 0, type = 'quiz', is_anonymous=False)
    await message.answer_poll( 
        question= '1️⃣первый', 
        open_period = 10,
        options= [input_poll_option.InputPollOption(text= 'impossible', voter_count = 10), 
                    input_poll_option.InputPollOption(text= 'inappropriate', voter_count= 10), 
                    input_poll_option.InputPollOption(text= 'inevitable', voter_count= 10)], 
        correct_option_id= 0,
        #is_anonimous = False,
        type = 'quiz',
        explanation = explanation, 
        explanation_entities = [MessageEntity(type = 'text_mention', offset = 0, length = len(explanation), user = User(id = message.from_user.id, is_bot = False, first_name= 'v'), extra_data = {'id': 155269575}),
                                MessageEntity(type = 'custom_emoji', offset = 10, length = 0, custom_emoji_id = '23', extra_data = {'id': 155269575})],
        extra_data = '{"id": "155269575"}'                        
        )
...
async def poll_answer_handler(poll: Poll):
    ...
    for i in range(len(poll.options)):
        ...
        if poll.options[i].voter_count == 1 and i == poll.correct_option_id:
            rightans = True
    if  voter_count > 1 or voter_count == 0:
            return   
    if not rightans:
...
async def Choose_one(message: Message):
    ...
    task_level = await rq.db_helper(message.from_user.id, task_param = 'question_level_time', command= 'test2')
    if task_level:
        await message.answer(text= await rq.three_options_sentence(int(task_level), Test2), reply_markup= await rq.three_options_keyboard(int(task_level), Test2))
    else:
        await message.answer('Новый тест пока что недоступен')    
...
async def my_callback(query: CallbackQuery, callback_data: Control):
    ...
    if callback_data.id%20 == 0:
        ...
        await query.message.edit_text(text=f'Тест пройден. Здесь анализ ответов исходя из  {callback_data.rightans}')
    else:    
        await query.message.edit_text(text= await rq.three_options_sentence(callback_data.id + 1, Test2), reply_markup= await rq.three_options_keyboard(callback_data.id + 1, Test2, callback_data.rightans))
...
async def compilesentence(message: Message):
    task_level = True #await rq.db_helper(message.from_user.id, task_param = 'words_level_time', command= 'words_level_time')
    if task_level:
        await message.answer(' ?', reply_markup= await rq.collect_words(id = 1, table = Question))
    else:
...
async def my_callback(query: CallbackQuery, callback_data: Questions):
    ...
    if ' ' + word + ' ' in query.message.text or word + ' ?' in query.message.text: # протестить
        ...
        await query.message.edit_text(text= newline, reply_markup= await rq.collect_words(id = callback_data.id, rightans= callback_data.rightans, indexes = indexes, current_indexes = current_indexes, table = Question))
    else:  
        current_indexes.append(callback_data.index) 
        await query.message.edit_text(text = query.message.text.rstrip('?') +  callback_data.word + ' ?' , reply_markup= await rq.collect_words(id = callback_data.id, rightans= callback_data.rightans, indexes = indexes, current_indexes = current_indexes, table = Question), parse_mode= 'MarkdownV2')
        ...
        if len(indexes) == len(current_indexes) - 1:
            ...
            if callback_data.id % 10 == 0:
                ...
                await query.message.edit_text(' ?', reply_markup= await rq.collect_words(id = callback_data.id + 1, rightans= callback_data.rightans, table = Question))  
