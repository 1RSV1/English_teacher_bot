...
import os
import math
import time, json, requests, string, subprocess
import random
...
async def handle_all_audios(message: Message):
    ...
    audio.export(f'audio/{message.from_user.id}.oga', format='ogg', codec='libvorbis')
    '''
    command = [
        'ffmpeg',
        '-i', f'audio/{message.from_user.id}.mp3',
        '-c:a', 'libvorbis',
        '-q:a', '6',  # Quality level (1-10, 10 is highest)
        f'audio/{message.from_user.id}.ogg'
    ]
