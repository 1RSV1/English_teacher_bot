...
from aiogram.types import Message, CallbackQuery, InputMediaPhoto, Video, InputMediaVideo, ForceReply, LabeledPrice, PreCheckoutQuery, input_poll_option, PollAnswer
...
async def poll_answer_handler(poll_answer: PollAnswer):
    ...
    await bot.send_poll(chat_id = poll_answer.user.id, question= 'new new', options= [input_poll_option.InputPollOption(text= 'impossible', voter_count = 10), input_poll_option.InputPollOption(text= 'inappropriate', voter_count= 10), input_poll_option.InputPollOption(text= 'inevitable', voter_count= 10)], correct_option_id= 0, type = 'quiz', is_anonymous=False)
    await bot.delete_message(chat_id = poll_answer.user.id, message_id= 3918)
